---
title: "Lab 02 - Plastic waste"
author: "Sabrina Eisenberg"
date: "1/27/2022"
output: html_document
---

## Load packages and data

```{r setup, message=FALSE}
library(tidyverse)
```

```{r load-data, message=FALSE}
plastic_waste <- read_csv(here::here("data", "plastic-waste.csv"))
```

## Warm up 
Notice that some cells in the data have the value NA - what does this mean? 

This is where there are missing values 

## Exercises

```{r plastic waste per cap dist}

ggplot(data = plastic_waste) +
  aes(x = plastic_waste_per_cap) +
  geom_histogram(binwidth = 0.2)

```

```{r filter greater than 3.5}
plastic_waste |>
  filter(plastic_waste_per_cap > 3.5)
```
This code finds the data points that are greater than the specified data point (3.5)

My brief research shows that Trinidad is a small island with many peopel that does not have a recycling system proportionate to its waste production. 

### Exercise 1


```{r plastic-waste-continent}

ggplot(data = plastic_waste) +
  aes(x = plastic_waste_per_cap) +
  geom_histogram(binwidth = 0.2) + 
  facet_wrap (~continent)
  

```

```{r plastic-waste-density}

ggplot(data = plastic_waste) +
  aes(x = plastic_waste_per_cap) +
  geom_density() +
  facet_wrap(~continent)

```

```{r plastic-waste-density-hist}

ggplot(data = plastic_waste) +
  aes(x = plastic_waste_per_cap) +
  geom_density(aes(y = after_stat(count), fill = continent, color = continent), alpha = 0.1) 

```


### Exercise 2

```{r plastic-waste-box-plot}

ggplot(data = plastic_waste) +
  aes(x = continent,
      y = plastic_waste_per_cap,
      fill = continent) +
  geom_jitter(height = 0,
              width = .4) +
  geom_boxplot(color = "black",
               alpha = .5) +
    ggdist::stat_slab(height = .3,
                    color = "black",
                    size = .2,
                    alpha = .5,
                    position = position_nudge(y = .2))

```

The density plot and jitter show the distribution of the data points better than the box plot. It accounts more for outliers as well. 

### Exercise 3

```{r plastic waste-mismanaged scatter}

ggplot(data = plastic_waste) +
  aes(x = plastic_waste_per_cap,
      y = mismanaged_plastic_waste_per_cap, color = continent) +
  geom_point()

```

1. Describe the relationship
It seems like there is a positive relationship between the two variables with increasing variability. 

2. There seems to be a steeper positive relationship for Africa and Asia. North America, Europe, and Oceana seem to have more variability. Europe and North America's positive relationship seems to have a smaller slope. 

3. 

```{r plastic waste total pop}

ggplot(data = plastic_waste) +
  aes(x = total_pop,
      y = plastic_waste_per_cap) +
  geom_point() +
  geom_smooth()

```

```{r plastic waste total coastal pop}

ggplot(data = plastic_waste) +
  aes(x = coastal_pop,
      y = plastic_waste_per_cap) +
  geom_point() +
  geom_smooth()

```

# Which has better fit? 

### Exercise 4

Remove this text, and add your answer for Exercise 4 here.

```{r plastic-waste-violin}
# insert code here
```

### Exercise 5

Remove this text, and add your answer for Exercise 5 here.

```{r plastic-waste-mismanaged}
# insert code here
```

### Exercise 6

Remove this text, and add your answer for Exercise 6 here.

```{r plastic-waste-mismanaged-continent}
# insert code here
```

### Exercise 7

Remove this text, and add your answer for Exercise 7 here.

```{r plastic-waste-population-total}
# insert code here
```

```{r plastic-waste-population-coastal}
# insert code here
```

### Exercise 8

Remove this text, and add your answer for Exercise 8 here.

```{r recreate-viz}
# insert code here
```

## Pro-Tips

### Excercise 3

Try this :D

```
ggplot(data = plastic_waste, 
       mapping = aes(x = continent, 
                     y = plastic_waste_per_cap)) +
  geom_violin()+
  geom_boxplot(width=.3, fill="green") +
  stat_summary(fun.y=median, geom="point") 
```

### Exercise 5 

Helpful reference:http://www.sthda.com/english/wiki/ggplot2-themes-and-background-colors-the-3-elements
