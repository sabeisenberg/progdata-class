...
```{r selecter year lifeExp and country}
select(gapminder, year, lifeExp, country)
```

2. Select all variables, from `country` to `lifeExp`.

```{r select all variables country lifeExp}
# This will work:
...
```{r put continent first}
select(gapminder, continent, everything(gapminder))
```

5. Rename `continent` to `cont`.

```{r rename continent to cont}
# compare
select(gapminder, cont = continent) #new_name = old_name, this will only select continent 

rename(gapminder, cont = continent, Year = year) # this will produce the whole data set with the replaced name 
```
...
```{r order by year and then lifeExp}
...
```{r filter pop greater than 100 mill}
gapminder |>
...
```{r keep Asian greater 100 mill}
gapminder |>
  filter(continent == "Asia", pop > 100000000) #can also use & 
```

3. Only keep data _from the countries Brazil and China_ with population greater than 100 million.

```{r Brazil China pop 100 mill}
#gapminder |>
 # filter((country == "Brazil" & country == "China") & pop > 100000000)

# gapminder |>
#  filter((country == c("Brazil", "China")) & pop > 100000000)
# with this, it will ask first "is this equal to china" for 1, "is this equal to brazil" for 2. It will ask one at a time, so there will be missing data. If you want "any of these" use %in%
...
gapminder |>
  filter(country %in% allowed_countries & pop > 100000000)
```
...
gapminder |>
  mutate(lifeExp = if_else(
        country == "Canada" & year == 1952, 
        true = 70, 
        false = lifeExp))

#in the condition that country is Canada and year is 1952, keep the lifeExp if it is 70 but false if it is not. 
